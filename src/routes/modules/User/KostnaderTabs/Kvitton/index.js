...
import { getUsersInkop,downloadFile,deleteInkop } from 'redux/actions';
...
function Kvitton(props) {
    ...
    const userInkop = useSelector(({ usersReducer }) => usersReducer.userInkop);
   ...
   const handlePageChange = (event, newPage) => {
    ...
    const handleRequestSort = (event, property) => {
     ...
     React.useEffect(()=>{
        ...
        dispatch(getUsersInkop(currentUser.id,searchTerm,isEmpty(filterOptions)?[{date:moment(new Date()).format("YYYY-MM-DD")}]:filterOptions),
        () => {
      ...
      React.useEffect(()=>{
        if(isEmpty(userInkop)){setTotal(0)}
        else{
          let Totalr= 0
          userInkop.map(item=>{
      ...
      const handlefileDelete=(fileId,Url)=>{      
          dispatch(deleteInkop(currentUser.id,fileId,Url,searchTerm,isEmpty(filterOptions)?[{date:moment(new Date()).format("YYYY-MM-DD")}]:filterOptions))
      }
    ...
    return (
        <div className={classes.root}>
            ...
            <TableContainer className={classes.container}>
                ...
                <TableBody>
                {!!userInkop.length ? (
                   stableSort(userInkop, getComparator(order, orderBy))
                  ...
                  <TableCell colSpan={props.isMobile?5:8} rowSpan={10}>
                    {isFilterApplied ? (
                      <NoRecordFound>There are no records found with your filter.</NoRecordFound>
                    ) : (
                      <NoRecordFound>{userInkop ? 'There are no records found.' : 'Loading...'}</NoRecordFound>
                    )}
                  </TableCell>
                </TableRow>
                )}
