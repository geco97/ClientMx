...
import { getUsersKvitto,downloadFile,deleteKvitto } from 'redux/actions';
...
function Kvitton(props) {
  //userKvitto
    ...
    React.useEffect(()=>{
      ...
      dispatch(getUsersKvitto(currentUser.id,searchTerm,isEmpty(filterOptions)?[{date:moment(new Date()).format("YYYY-MM-DD")}]:filterOptions),
      () => {
    ...
    React.useEffect(()=>{
      if(isEmpty(userKvitto)){setTotal(0)}
      else{
        let Totalr= 0
        userKvitto.map(item=>{
    ...
    const handlefileDelete=(fileId,Url)=>{      
      dispatch(deleteKvitto(currentUser.id,fileId,Url,searchTerm,isEmpty(filterOptions)?[{date:moment(new Date()).format("YYYY-MM-DD")}]:filterOptions))
    }
    return (
        <div className={classes.root}>
            ...
            <TableContainer className={classes.container}>
                ...
                rowCount={userKvitto.length}
                />
                <TableBody>
                {!!userKvitto.length ? (
                   stableSort(userKvitto, getComparator(order, orderBy))
                  ...
                  <TableCell colSpan={props.isMobile?3:6} rowSpan={10}>
                    {isFilterApplied ? (
                      ...
                      <NoRecordFound>{userKvitto ? 'There are no records found.' : 'Loading ...'}</NoRecordFound>
                    )}
                  </TableCell>
                </TableRow>
                )}
