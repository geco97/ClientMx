...
function AddEditEvent({ open, onCloseDialog, Cevent,Add }) {
    ...
    const onDeleteClick =()=>{
        ...
        dispatch(onDeleteEvent(event,currentUser.id,month,year));
        onCloseDialog()
    }
    ...
    const onSubmitClick = ()=>{
        ...
        if(parseInt(Add) === 1){
            ...
            dispatch(EditEvent(Event,currentUser.id,month,year));
        }
        else{
            let nItems=[]
            ...
            dispatch(AddnewEvent(Event,currentUser.id,month,year));
        }
        onCloseDialog()
    }
                   ...
                   event.Itemms.map(item=>{
                        ...
                        <option key={5} value="Sjuk">Sjuk</option>
                           ...
                           disabled={isEmpty(event.Itemms.filter(itemff=>{return itemff.start === item.start 
                            && (itemff.Status === "Arbete" || itemff.Status === "VAB" || itemff.Status === "Sjuk" )
                        }))}
                           maskChar=" "
