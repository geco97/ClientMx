...
import HeaderStatistic from '../../../../Apps/Tidrapportering/Calendar/HeaderStatistic'
const {
  getHolidays,
...
const PopupCalendar = () => {
    ...
    React.useEffect(()=>{
        dispatch(onGetUserEvent(currentUser.id,crrentmonth,currentYear))
    },[])
    React.useEffect(()=>{
  ...
  return (
    <PageContainer>
      ...
      <Calendar 
        ...
        style={{ width: '98%', height:"400px",fontSize:12,margin: 'auto', padding: '1rem 0' }}
        localizer={localizer}
        events={Events} 
        messages={messages}
        views={['month']}     
        ...
        onRangeChange={e => {
            ...
            console.log(middle);
            dispatch(onGetUserEvent(currentUser.id,moment(middle).format("MM"),moment(middle).format("YYYY")))
        }}
        popup />
