...
const tabs = [
    { id: 1, title: 'Profil', slug: 'About' },
    { id: 2, title: 'Kvitton', slug: 'Intakter' },
    { id: 6, title: 'Fakturor', slug: 'Fakturor' },
    { id: 3, title: 'Inköp', slug: 'Kostnader' },
    { id: 5, title: 'TidRapportering', slug: 'TidRapport' },
    { id: 7, title: 'Dokument', slug: 'Dokument' },
  ];
  ...
  const Header = ({ userDetail, tabValue, handleTabChange }) => {
    const { Firstname,Lastname, Email,BildUrl } = userDetail;
          ...
          <Box display="flex" flexDirection={{ xs: 'column', sm: 'row' }} alignItems="center" mb={4}>
            <Box display="flex" flexDirection={{ xs: 'column', sm: 'row' }} alignItems="center">
              <Box mr={{ sm: 4, md: 5, lg: 6 }} mb={{ xs: 3, sm: 0 }}>
                <CmtAvatar size={80} src={BildUrl} alt={`${Firstname} ${Lastname}`} />
              </Box>
              <Box>
                <Typography className={classes.titleRoot} component="div" variant="h1">
                  {`${Firstname} ${Lastname}`}
                </Typography>
                <Typography className={classes.subTitleRoot}>{isEmpty(Email)?"InAktiv":Email}</Typography>
              </Box>
            </Box>
          </Box>
  
          <Box display="flex" justifyContent={{ xs: 'center', sm: 'flex-end' }}>
            <Tabs className={classes.tabsList} value={tabValue} 
             scrollButtons="auto"
             variant="scrollable"
            onChange={handleTabChange} aria-label="profile tabs">
              {tabs.map((item, index) => {
                return (
                  <Tab
