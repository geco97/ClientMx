...
import { AddnewFaktura } from 'redux/actions';
...
function AddEditInvoice({ open, onCloseDialog,searchTerm }) {
    ...
    const onSubmitClick = ()=>{
        if(!isEmpty(file)){
            dispatch(AddnewFaktura(currentUser.id,date,Value,file,searchTerm));
            onCloseDialog()
        }
    }
    return (
        <>
            ...
            open={open} onClose={onCloseDialog} className={classes.dialogRoot}>
                <DialogTitle className={classes.dialogTitleRoot}>{'Ny Faktura'}</DialogTitle>
